 ...
 *  - Marca de agua diagonal con el wordmark RISKS al fondo.
 *  - Banda lateral izquierda navy con leyenda "ISO/IEC 17024" rotada —
 *    refuerzo de identidad sin sobrecargar.
 *  - QR animado con patrón pseudo-aleatorio + halo "ping" que llama la
 *    atención a la verificación pública (CTA implícito).
 *  - Sello "VERIFICADO" con micro-shimmer y micro-rotación al hover.
 *  - Entrada con fade-in + scale al cargar; flotación continua suave.
 *
 * Las animaciones declaradas aquí usan utilidades extra agregadas en
 * src/app/globals.css (animate-shimmer, animate-ping-soft, etc.).
 */
export async function CertificateMock({ tilt = true }: { tilt?: boolean }) {
  ...
  return (
    <div className="certificate-mock relative mx-auto w-full max-w-md select-none">
      {/* Capa "sombra de papel" detrás — un certificado abajo y otro encima
          dan sensación de "expediente" con varias hojas. */}
      <div
        aria-hidden
      ...
      >
        {/* Banda lateral izquierda navy con texto rotado — refuerzo de marca */}
        <span
          aria-hidden
          className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 -rotate-90 font-mono text-[8px] font-bold uppercase tracking-[0.4em] text-brand-900/40"
        >
          ISO/IEC 17024 · {BRAND.shortName}
        </span>

        {/* Marca de agua principal en diagonal */}
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center overflow-hidden rounded-2xl">
          <span className="rotate-[-20deg] text-[80px] font-black tracking-tighter text-brand-900 opacity-[0.04]">
            RISKS
          </span>
        </div>

        {/* Marco interno — antes era "border-gold-fine"; ahora navy */}
        <div className="relative m-3 rounded-xl ring-1 ring-brand-900/15">
          ...
          <div className="rounded-lg p-5 text-center">
            ...
            {/* Etiqueta superior — antes con borde dorado; ahora navy.
                Animación shimmer ligera para dar sensación de "sello" premium. */}
            ...
            <p className="mt-1 text-lg font-bold tracking-tight text-brand-900">Nombre del Profesional</p>
            ...
            <div className="mt-5 flex items-end justify-between gap-3">
              ...
              {/* QR + estado vigente.
                  El QR tiene un halo "ping" sutil que llama la atención sin
                  ser molesto (suspensión cada ~4s). */}
                    ...
                    <div className="absolute inset-1 grid grid-cols-5 gap-px">
                      {Array.from({ length: 25 }).map((_, i) => (
                        <span
                          key={i}
                          className={
                            i % 3 === 0 || i === 7 || i === 12 || i === 17 ? "bg-white" : "bg-transparent"
                          }
                        />
                      ))}
                    </div>
      ...
      {/* Sello "VERIFICABLE" — antes dorado; ahora navy con shimmer y hover-tilt. */}
      <span
        aria-hidden
